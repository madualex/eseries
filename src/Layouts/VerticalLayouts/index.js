...
const VerticalLayout = (props) => {
    ...
    return (
        <React.Fragment>
            ...
            {(navData || []).map((item, key) => {
                return (
                    <React.Fragment key={key}>
                        {/* Main Header */}
                        {item['isHeader'] ?
                            <li className="px-5 menu-title text-white"><span data-key="t-menu">{props.t(item.label)} </span></li>
                            : (
                                (item.subItems ? (
                                    ...
                                    <li className="text-white mx-4 nav-item">
                                        <Link
                                            className="nav-link menu-link rounded text-white menu__hover active"
                                                to={item.link ? item.link : "/#"}
                                        >
                                            <i className={`fs-3 ${item.icon}`}></i> <span className='ms-3 fs-6'>{props.t(item.label)}</span>
                                        </Link>
                                    </li>
                                ))
                            )
            ...
            <Link to='/logout' className='mx-4 pe-5 pt-2 bg-white d-flex rounded justify-content-center' style={{ marginTop: '50px' }}>
                <i className='fs-4 ri-logout-circle-r-line text-success'></i>
                <p className='text-success pt-1 ms-3 fw-semibold'>Logout</p>
            </Link>
        </React.Fragment>
    );
};
